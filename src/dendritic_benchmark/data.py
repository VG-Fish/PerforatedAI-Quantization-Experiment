...
SPEECH_COMMAND_LABELS = ("yes", "no", "up", "down", "left", "right", "on", "off", "stop", "go", "zero", "one")
...
def _make_loader(dataset: Any, batch_size: int, shuffle: bool = False) -> Any:
    torch = require_torch()
    return torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=shuffle)


def _split_dataset(dataset: Any, train_ratio: float = 0.7, val_ratio: float = 0.15) -> tuple[Any, Any, Any]:
    torch = require_torch()
    total = len(dataset)
    if total < 3:
        raise ValueError("Need at least three samples to build train/validation/test splits.")
...
def _bundle_from_splits(
    ...
    return TaskBundle(
        ...
        _make_loader(val_ds, batch_size),
        _make_loader(test_ds, batch_size),
        metric_name,
        metric_direction,
        input_description,
    )
...
def _bundle_from_dataset(
    ...
    return _bundle_from_splits(*_split_dataset(dataset), batch_size, metric_name, metric_direction, input_description)
...
def _build_mnist(batch_size: int) -> TaskBundle:
    ...
    train_full = torchvision.datasets.MNIST(root=str(root), train=True, download=True, transform=transform)
    test_ds = torchvision.datasets.MNIST(root=str(root), train=False, download=True, transform=transform)
    train_ds, val_ds = require_torch().utils.data.random_split(
        train_full,
        [55_000, 5_000],
        generator=require_torch().Generator().manual_seed(42),
    )
    return _bundle_from_splits(train_ds, val_ds, test_ds, batch_size, "Accuracy", "maximize", "MNIST handwritten digit images")
...
class _SpeechCommands12:
    def __init__(self, subset: str):
        ...
        root.mkdir(parents=True, exist_ok=True)
        base = torchaudio.datasets.SPEECHCOMMANDS(str(root), download=True, subset=subset)
    ...
    def __getitem__(self, index: int):
        ...
        if sample_rate != self.target_len:
            waveform = self.torch.nn.functional.interpolate(
        ...
        if waveform.shape[-1] < self.target_len:
            waveform = self.torch.nn.functional.pad(waveform, (0, self.target_len - waveform.shape[-1]))
        waveform = waveform[:, : self.target_len]
        return waveform, self.torch.tensor(self.labels[label], dtype=self.torch.long)
...
def _build_speechcommands(batch_size: int) -> TaskBundle:
    ...
    return _bundle_from_splits(train_ds, val_ds, test_ds, batch_size, "Accuracy", "maximize", "SpeechCommands 12-class keyword audio")
...
def _build_etth1(batch_size: int) -> TaskBundle:
    ...
    seq_len = 24
    x = torch.stack([values[index : index + seq_len] for index in range(len(values) - seq_len)]).unsqueeze(-1)
    y = torch.stack([values[index + seq_len] for index in range(len(values) - seq_len)])
    return _bundle_from_dataset(_TensorRowsDataset(x, y), batch_size, "MAE", "minimize", "ETTh1 hourly oil temperature forecasting windows")
...
def _build_vocab(texts: Iterable[str], vocab_size: int) -> dict[str, int]:
    ...
    return {token: index + 1 for index, (token, _) in enumerate(counts.most_common(vocab_size - 1))}
...
def _build_ag_news(batch_size: int) -> TaskBundle:
    ...
    return _bundle_from_splits(train_ds, val_ds, _TensorRowsDataset(x_test, y_test), batch_size, "Accuracy", "maximize", "AG News tokenized article titles and descriptions")
...
def _build_sst2(batch_size: int) -> TaskBundle:
    ...
    return _bundle_from_splits(train_ds, val_ds, _TensorRowsDataset(x_test, y_test), batch_size, "Accuracy", "maximize", "SST-2 tokenized sentiment sentences")
...
def _build_cora(batch_size: int) -> TaskBundle:
    ...
    class CoraEgoDataset:
        def __len__(self) -> int:
            return len(paper_ids)

        def __getitem__(self, index: int):
            neighbors = adjacency[index].nonzero().flatten()[:50]
            if len(neighbors) < 50:
                pad = neighbors.new_full((50 - len(neighbors),), index)
                neighbors = torch.cat([neighbors, pad])
            sub_x = x_all[neighbors]
            sub_adj = adjacency[neighbors][:, neighbors]
            return sub_x, sub_adj, y_all[index]

    return _bundle_from_dataset(CoraEgoDataset(), batch_size, "Accuracy", "maximize", "Cora citation-network ego graphs for node labels")
...
def _build_adult(batch_size: int) -> TaskBundle:
    ...
    train_ds, val_ds, _ = _split_dataset(
        _TensorRowsDataset((train_x - mean) / std, torch.tensor(train_y_raw, dtype=torch.long)),
        train_ratio=0.9,
        val_ratio=0.1,
    )
    test_ds = _TensorRowsDataset((test_x - mean) / std, torch.tensor(test_y_raw, dtype=torch.long))
    return _bundle_from_splits(train_ds, val_ds, test_ds, batch_size, "Accuracy", "maximize", "Adult Income census features")
...
def _build_esol(batch_size: int) -> TaskBundle:
    ...
    with path.open(newline="") as fh:
        ...
        for row in reader:
            ...
            target = row.get("measured log solubility in mols per litre") or row.get("ESOL predicted log solubility in mols per litre")
    ...
    return _bundle_from_dataset(
        _TensorRowsDataset(torch.stack(node_features), torch.stack(adjacencies), torch.tensor(labels, dtype=torch.float32)),
        batch_size,
        "RMSE",
        "minimize",
...
def _build_cartpole(batch_size: int) -> TaskBundle:
    ...
    return _bundle_from_dataset(_TensorRowsDataset(x, y), batch_size, "Reward", "maximize", "CartPole-v1 observations labeled by a stabilizing policy")
...
def _build_mitbih(batch_size: int) -> TaskBundle:
    ...
    return _bundle_from_dataset(
        _TensorRowsDataset(torch.stack(windows), torch.stack(windows), torch.tensor(labels, dtype=torch.long)),
        batch_size,
        "AUC",
        "maximize",
...
def build_task_bundle(model_key: str, batch_size: int = 32) -> TaskBundle:
    require_torch()
    builders = {
        "lenet5": _build_mnist,
    ...
    return builders[model_key](batch_size)
